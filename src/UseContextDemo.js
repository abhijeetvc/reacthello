...
function UseContextDemo() {
  ...
  return (
    <MyContext.Provider
      value={{ number, selectedUser, setSelectedUser, users, setUsers }}
    >
      <div>
        ...
        <ul>
          {selectedUser && 
            selectedUser.products.map((p) => <li>{p.product}</li>)}
        </ul>
        {/* <ul>
          {users.map((user) => (
            <div>
              <li>
                <a onClick={() => getUser(user)}>{user.name}</a>
              </li>
            </div>
          ))}
        </ul> */}
...
function CheckContext() {
  const { number, selectedUser, setSelectedUser, users, setUsers } =
